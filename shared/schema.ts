...
export const partnerWebhookConfigs = pgTable("partner_webhook_configs", {
  ...
  notifyViaQueue: boolean("notify_via_queue").default(true).notNull(),
  notifyViaWebhook: boolean("notify_via_webhook").default(true).notNull(),
  createdAt: timestamp("created_at").default(sql`now()`),
  updatedAt: timestamp("updated_at").default(sql`now()`),
});
...
export const aiAssistantPreferences = pgTable("ai_assistant_preferences", {
  ...
  preferredModel: text("preferred_model").$type<"gemini-2.5-pro" | "gemini-3-flash-preview" | "gemini-3.1-pro-preview">().default("gemini-3-flash-preview"),
...
export const appointmentBookings = pgTable("appointment_bookings", {
  ...
  source: text("source").$type<"whatsapp" | "public_link" | "instagram" | "public_page" | "voice_call">().default("whatsapp").notNull(),
  voiceCallId: varchar("voice_call_id"),
...
export const fileSearchStores = pgTable("file_search_stores", {
  ...
  ownerType: text("owner_type").$type<"consultant" | "client" | "system" | "whatsapp_agent" | "email_account" | "autonomous_agent" | "department">().notNull(),
...
export const fileSearchSettings = pgTable("file_search_settings", {
  ...
  autoSyncExerciseExternalDocs: boolean("auto_sync_exercise_external_docs").default(false).notNull(),
  ...
  autoSyncOperationalTwilioTemplates: boolean("auto_sync_operational_twilio_templates").default(false).notNull(),
  autoSyncOperationalConfig: boolean("auto_sync_operational_config").default(false).notNull(),
  autoSyncOperationalEmail: boolean("auto_sync_operational_email").default(false).notNull(),
  autoSyncOperationalCampaigns: boolean("auto_sync_operational_campaigns").default(false).notNull(),
  autoSyncOperationalCalendar: boolean("auto_sync_operational_calendar").default(false).notNull(),
  autoSyncOperationalExercisesPending: boolean("auto_sync_operational_exercises_pending").default(false).notNull(),
  autoSyncOperationalConsultations: boolean("auto_sync_operational_consultations").default(false).notNull(),
  operationalSyncEnabled: boolean("operational_sync_enabled").default(false).notNull(),
  operationalSyncIntervalMinutes: integer("operational_sync_interval_minutes").default(30).notNull(),
  lastOperationalSyncAt: timestamp("last_operational_sync_at"),
...
export const contentIdeas = pgTable("content_ideas", {
  ...
  mediaType: varchar("media_type", { length: 20 }).default("photo").$type<"video" | "photo">(),
  copyType: varchar("copy_type", { length: 20 }).default("short").$type<"short" | "long">(),
...
export const contentIdeaTemplates = pgTable("content_idea_templates", {
  ...
  additionalContext: text("additional_context"),
  awarenessLevel: varchar("awareness_level", { length: 50 }),
...
export const contentPosts = pgTable("content_posts", {
  ...
  structuredContent: jsonb("structured_content").$type<Record<string, unknown>>().default(sql`'{}'::jsonb`),
  mediaType: varchar("media_type", { length: 50 }).$type<"video" | "foto">(),
  copyType: varchar("copy_type", { length: 50 }).$type<"long" | "short">(),
  videoHook: text("video_hook"),
  videoProblema: text("video_problema"),
  videoSoluzione: text("video_soluzione"),
  videoCta: text("video_cta"),
  videoFullScript: text("video_full_script"),
  imageDescription: text("image_description"),
  imageOverlayText: text("image_overlay_text"),
  chiCosaCome: text("chi_cosa_come"),
  errore: text("errore"),
...
export const clientDataDatasets = pgTable("client_data_datasets", {
  ...
  columnMapping: jsonb("column_mapping").$type<Record<string, { displayName: string; dataType: string; description?: string }>>().default({}),
...
export const consultantColumnMappings = pgTable("consultant_column_mappings", {
  id: serial("id").primaryKey(),
  consultantId: varchar("consultant_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  originalColumn: varchar("original_column", { length: 255 }).notNull(),
  mappedColumn: varchar("mapped_column", { length: 255 }).notNull(),
  mappedType: varchar("mapped_type", { length: 50 }).notNull().$type<"TEXT" | "NUMERIC" | "INTEGER" | "DATE" | "BOOLEAN">(),
...
export const clientDataMetrics = pgTable("client_data_metrics", {
  ...
  formulaSql: text("formula_sql"),
  resultType: varchar("result_type", { length: 50 }).$type<"currency" | "percentage" | "number" | "count">(),
  unit: varchar("unit", { length: 50 }).$type<"currency" | "percentage" | "number" | "count">(),
...
export const clientDataQueryLog = pgTable("client_data_query_log", {
  ...
  userQuestionHash: varchar("user_question_hash", { length: 64 }),
  intentType: varchar("intent_type", { length: 50 }).$type<"analytical" | "informational" | "operational">(),
  toolName: varchar("tool_name", { length: 100 }),
  toolParams: jsonb("tool_params").$type<Record<string, any>>(),
  toolCallJson: jsonb("tool_call_json").$type<any>(),
  sqlExecuted: text("sql_executed"),
  resultSummary: jsonb("result_summary").$type<any>(),
  ...
  userIdx: index("idx_client_data_query_log_user").on(table.userId),
}));
...
export const contentStudioConfig = pgTable("content_studio_config", {
  ...
  // Market Research Data (7-phase deep research, per-consultant global copy)
  marketResearchData: jsonb("market_research_data").$type<MarketResearchData>().default(sql`'{}'::jsonb`),
...
export const aiAutonomySettings = pgTable("ai_autonomy_settings", {
  ...
  autonomyModel: varchar("autonomy_model", { length: 50 }).default("gemini-3-flash-preview"),
  autonomyThinkingLevel: varchar("autonomy_thinking_level", { length: 10 }).default("low"),
