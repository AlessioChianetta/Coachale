...
- **Email Hub System**: Complete email management for consultants with IMAP/SMTP support, unified inbox viewing, and AI-powered response generation. Features: 4 account types (smtp_only, imap_only, full, hybrid) for flexible provider configurations (e.g., Amazon SES SMTP + Register.it IMAP), provider classification system detecting send-only services (SES, SendGrid, Mailgun) vs full-service providers (Gmail, Outlook), 3-step ImportWizardDialog for guided account configuration, Italian provider presets (Register.it, Aruba, Mailserver.it, OVH), sidebar UI with account type icons showing capabilities, multi-folder support (Inbox, Sent, Drafts) with automatic background synchronization on account import, automatic IMAP IDLE activation for real-time email reception, folder-based query filtering in GET /inbox endpoint with `folder` and `direction` parameters, AI email classification and draft generation, **universal IMAP folder discovery** using RFC 6154 SPECIAL-USE attributes with localized fallback aliases (Italian: "Posta Inviata", French: "Envoyés", Spanish: "Enviados", German: "Gesendet", Dutch: "Verzonden", Portuguese: "Enviadas") for worldwide IMAP server compatibility, folder paths stored in database columns (inbox_folder_path, sent_folder_path, drafts_folder_path, etc.) for persistent sync, **scalable email import system** for large mailboxes (100k+ emails) with `email_import_jobs` table for per-folder progress tracking, batch processing (default 100 emails/batch), SSE real-time progress streaming, pause/resume/cancel controls, ETA calculation, and EmailImportDialog UI with Italian labels, **improved IDLE connection reliability** with max 5 reconnect attempts, exponential backoff (5-25s), automatic database `syncStatus` update to 'error' on permanent connection failure, and `shouldReconnect` flag to prevent reconnect loops after intentional disconnect. Routes: `/consultant/email-hub` for the main interface.
