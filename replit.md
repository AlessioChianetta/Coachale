...
- Content Autopilot System: Batch generation of social media posts similar to Email Nurturing 365. Features include: date range selection, platform-specific frequency (1-3 posts/day), SSE progress streaming, automatic scheduling with calendar integration. Posts are generated per-platform with appropriate writing styles (Conversational for Instagram, Direct for X, Default for LinkedIn). Posting schedule configuration available in Brand Assets with customizable times and styles per platform.
