...
- Voice Telephony System (Alessia AI Phone) with FreeSWITCH integration, automatic caller recognition, Gemini 2.5 Flash Native Audio for real-time voice AI, database tables for call management, rate limiting, service token authentication for VPS bridge, a Caller History Context System for returning anonymous callers, and a Voice Call Recording System with real-time transcript updates. **Dual Backend Live API (Feb 2026)**: Supports both Google AI Studio (`gemini-2.5-flash-native-audio-preview-12-2025` with thinking/+21% quality) and Vertex AI (`gemini-live-2.5-flash-native-audio` GA) with automatic fallback. Google AI Studio uses camelCase params + API key auth via `generativelanguage.googleapis.com`, Vertex AI uses snake_case params + OAuth2 Bearer via `aiplatform.googleapis.com`. Backend selection: Google AI Studio prioritized → Vertex AI fallback. **Backend-Specific Prompt Optimization (Feb 2026)**: Google AI Studio (7-chunk ~54K token limit) uses reordered context (consulenze/emails/Fathom transcripts BEFORE exercises) + compressed exercises (title+status only, no content/questions). Vertex AI (12+ chunks ~88K tokens, full context) keeps original order with full exercise details. Early backend detection via `LIVE_API_PROVIDER` env var determines optimization before context building. Greeting fix: explicit greeting trigger sent for ALL backends (both AI Studio and Vertex AI). Key files: `server/ai/provider-factory.ts` (`getGoogleAIStudioKeyForLive()`, `getVertexAITokenForLive()`), `server/ai/gemini-live-ws-service.ts` (dual setup message, dual audio format), `server/ai-prompts.ts` (`buildUserDataContextForLive` with `liveApiBackend` parameter).
...
- Direction-Based Voice Template System: Separate template configurations for INBOUND (non-client calls you) vs OUTBOUND (you call non-client) calls. Includes a template library (`server/voice/voice-templates.ts`) with 3 inbound templates (Mini-Discovery, Info Generale, Sales Call Orbitale Inbound) and 4 outbound templates (Sales Call Orbitale, Follow-up Lead, Recupero Crediti, Check-in Cliente). Each direction supports 3 prompt sources: Import from WhatsApp Agent, Manual Template, or Predefined Template. UI shows 2 separate cards for configuration. Templates use variable interpolation ({{consultantName}}, {{businessName}}, {{aiName}}, {{contactName}}). All booking-capable templates include a dedicated contact data FASE with CRM data confirmation flow.
...
- Voice Booking Supervisor (LLM Supervisor Architecture): Post-turn transcript analysis system (`server/voice/voice-booking-supervisor.ts`) that triggers bookings only when all data is complete and confirmed. Uses Gemini 2.5 Flash Lite for transcript analysis with a 5-stage state machine (nessun_intento → raccolta_dati → dati_completi → confermato → completato/errore). Dual-path booking: Path A for registered clients uses consultation-tools (proposeBooking→confirmBooking→consultation), Path B for leads uses booking-service (createBookingRecord→appointmentBooking). 6 architectural rules: (1) AI never claims booking before server confirmation, (2) mutex prevents double-triggers, (3) lastProposedSlot tracking prevents false confirmations, (4) brief slot catalog in prompts, (5) zero WhatsApp code changes, (6) comprehensive audit logging with delta indicators. UI shows green "Appuntamento" badge in call list and booking card in call detail page. Documentation in `docs/RDP-voice-booking-supervisor.md`.
- Anti-Zombie Connection System: Prevents stale Gemini WebSocket connections from consuming API quota. Features `lastActivity` tracking, 60-second garbage collector (IDLE_TIMEOUT: 30min, MAX_SESSION: 2h), client heartbeat ping/pong every 30s, and hard `terminate()` for immediate socket closure. Documentation in `docs/zombie-prevention-spec.md`. **Silent Session Detection (Feb 2026)**: Tracks user speech per Gemini session segment (`turnsInCurrentSegment`). If 2 consecutive resumes have zero user speech (~20 min silence), the call is automatically terminated to save API costs. Counter persists across resume boundaries via `silentStreak` URL parameter passed through the VPS bridge. Sends `call_terminated` message to bridge which triggers FreeSWITCH hangup. **Session Resume Fix (Feb 2026)**: Fixed critical bug where phone_service auth handler always returned `resumeHandle: null`, preventing VPS bridge proactive restarts from resuming Gemini sessions. Also fixed resume handle validation for anonymous callers (`userId: null` match) and ensured `suppressAiOutputAfterResume` activates for all resume modes (including phone calls without conversation history). Added `serverContent.interrupted` false-positive prevention (only sends `barge_in_detected` when `isAiSpeaking=true`). Added throttled diagnostic logs for silently discarded audio frames (server + VPS bridge).
