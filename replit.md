...
- **Per-Agent Google Calendar Integration**: Each WhatsApp agent can have its own dedicated Google Calendar for appointments, preventing conflicts between agents. Agent calendars are configured in the "Disponibilità & Automazioni" wizard step. Falls back to consultant's calendar if agent has no dedicated calendar. OAuth tokens stored per-agent in `consultant_whatsapp_config` table.
