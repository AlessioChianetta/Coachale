...
*   **Operational Data File Search Optimization:** When File Search is active, the consultant AI chat system prompt is reduced from ~125K tokens to ~8-10K tokens. 10 operational document generators sync data (clients, client states, WhatsApp/Twilio templates, config, email, campaigns, calendar, exercises, consultations) to File Search as `sourceType: "operational_context"`. The `buildConsultantContext()` function detects `fileSearchActive` and runs only 5 lightweight COUNT queries instead of 52 heavy queries. A `buildLightweightConsultantSystemPrompt()` produces a slim dashboard snapshot. Settings are managed per-consultant via `fileSearchSettings` table with individual toggle columns (`autoSyncOperational*`) and a master `operationalSyncEnabled` toggle. The cron scheduler (`dynamic-context-scheduler.ts`) passes operational settings to `syncDynamicDocuments()`. Frontend UI in `consultant-file-search-analytics.tsx` provides toggle controls and content display.
