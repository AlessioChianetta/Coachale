...
- **Manager Gold Memory System (Gold-Exclusive)**: AI memory for Gold tier managers stored in `manager_daily_summaries` table with same format as client memory. Features: automatic generation via cron scheduler (hourly), memory injection into system prompts only for Gold tier users, ManagerMemorySheet UI with Brain icon in navbar for Gold users to view their memory summaries, "Dipendenti Gold" section in consultant dashboard showing memory audit status for all Gold managers, manual memory generation per Gold manager via consultant dashboard, and tier-gated security ensuring Silver/Bronze users never receive memory context.
