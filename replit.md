...
- Content Studio Platform-Specific Schema Selection: Pre-generation configuration wizard with platform selection (Instagram/X/LinkedIn), post category (ads/valore/altri), and 40+ schema templates with structured formats (Hook→Problema→Soluzione→CTA, thread formats, case study templates). Character limits enforced per platform with X Premium subscription support (Instagram 2200, X free 280/X Premium 4000, LinkedIn 3000). Dynamic "Accorcia" (Shorten) button appears when content exceeds platform limits, using AI to intelligently shorten copy while preserving message, with intelligent paragraph-based fallback trimming. Platform filter in saved ideas UI. Full parameter passing to AI prompt for schema-aware content generation. Enhanced with strict schema enforcement ("seguire A PENNELLO") and mathematical length guarantees for long copy (minimum 250-375 chars per section ensuring 1500+ total). AI instructions now include explicit per-section requirements, concrete examples, and prominent enforcement blocks.
- Content Studio Writing Styles: 5 selectable writing styles for content generation - Predefinito (professional/balanced), Conversazionale/Nurturing (ultra-short 3-8 word sentences, line-by-line format, pattern interrupts like "Ma aspetta.", storytelling), Diretto (concise, no-fluff), Copy Persuasivo (copywriting triggers, urgency, power words), and Personalizzato (custom user instructions). Default copyType changed to 'long'. Writing style and custom instructions persisted in database for saved ideas.
...
- Content Studio Anti-Repetition System: Prevents AI from generating duplicate content by loading last 50 ideas + 30 posts and passing them to AI with explicit "VIETATO RIPETERE" rules. Temperature increased to 1.0 for greater variety.
- Content Autopilot System: Now integrated directly within the Ideas page (no standalone page). Features include: date range selection with past date blocking, platform-specific frequency (1-5 posts/day), 3 preset templates (Lancio Prodotto, Mantenimento, Campagna Aggressiva), SSE progress streaming, and automatic scheduling with calendar integration. Advanced filters: excludes weekends and Italian holidays (2024-2026 calendar), prevents duplicate generation for already-scheduled days with date range detection. Two generation modes: "Automatica" (batch all) and "Controllata" (day-by-day configuration with per-day controls). Content theme rotation (educativo, promozionale, storytelling, behind-the-scenes) with AI quality scoring (1-10) per post. Independent configuration UI: platform selector (Instagram/X/LinkedIn), category (ads/valore/altri), 40+ schema templates with dynamic availability based on platform+category, 5 writing styles + custom, media type (image/video/carousel/text), copy type (short/long). Controllata Mode: displays scrollable list of valid days with inline selectors for platform, category, schema, writing style, media type, and copy type per-day; supports single-day generation or batch generation of all pending days. Brand Assets Integration: loads posting schedule (times, posts per day) from Brand Assets with fallback to default optimal times. NEW: Full AdVisage AI integration with 3 advanced options: (1) "Genera Immagini AI" - automatically generates images using gemini-2.5-flash-image with SuperAdmin keys, (2) "Pubblica su Publer" - auto-publishes to Publer after generation, (3) "Modalità Review" - creates batch for approval before publishing. Character limits with 10% safety margin (Instagram 1980, LinkedIn 2700, X 252/3600). Retry loop (max 3 attempts) for posts exceeding character limits. Database tracking via autopilot_batches table with status (generating, awaiting_review, publishing, completed, failed) and post-level fields (autopilotBatchId, imageGenerationStatus, reviewStatus, charLimitRetries).
- AdVisage AI Visual Concept Generator: Integrated into Content Studio sidebar, AdVisage analyzes advertising copy and generates 3 AI-powered visual concepts with detailed image prompts, social captions, and platform-specific hashtags. Features include: multi-platform support (Instagram, TikTok, LinkedIn, Facebook), AI image generation using gemini-2.5-flash-image model, "Import from Posts" to load existing content from database with fullCopy prioritization, and "Upload to Publer" workflow to convert generated images to Publer media and optionally associate with existing posts via publerMediaIds. Factory mode for batch processing multiple ads, Pitch Deck mode for visual presentation, infinite scroll UX with localStorage persistence.
