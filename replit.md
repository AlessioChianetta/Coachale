...
- Content Studio Platform-Specific Schema Selection: Pre-generation configuration wizard with platform selection (Instagram/X/LinkedIn), post category (ads/valore/altri), and 40+ schema templates with structured formats (Hook→Problema→Soluzione→CTA, thread formats, case study templates). Character limits enforced per platform (Instagram 2200, X 280 tweet/1500 thread, LinkedIn 3000). Platform filter in saved ideas UI. Full parameter passing to AI prompt for schema-aware content generation. Enhanced with strict schema enforcement ("seguire A PENNELLO") and mathematical length guarantees for long copy (minimum 250-375 chars per section ensuring 1500+ total). AI instructions now include explicit per-section requirements, concrete examples, and prominent enforcement blocks.
