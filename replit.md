...
*   **Communication & Automation:**
    ...
    *   **Hunter Outreach System (Batch Campaigns):** Autonomous lead prospecting pipeline using SerpApi for Google Maps/Search and Firecrawl for enrichment. Hunter is the "commercial brain" — finds leads, qualifies them, decides channel/timing, then uses existing systems (WA Calendar, Voice Calendar, Email Hub) for outreach. Features: 4-step pipeline (Ricerca → Qualifica AI → Approvazione → Outreach), unified outreach queue with channel/status filters and batch approve, collapsible activity timeline, configurable multi-step follow-up sequences, post-call transcript AI classification with automatic next actions (wants_info→email, wants_appointment→CRM, interested_later→follow-up, not_interested→close, callback_later→reschedule), KPI conversion metrics, and Email Hub integration (outbound emails tracked in hub_emails). WA agent instructions are injected into outreach messages. Config includes `email_account_id` for email routing.
