...
# Recent Changes
- **[2026-02-17] AI Token Tracking System & Dashboard Restructure:**
  - **Token tracking**: All AI calls now routed through `trackedGenerateContent` in provider-factory.ts with feature tagging and real Gemini usageMetadata (no fallback estimates).
  - **Content Studio tracking**: 12 untracked AI calls in content-studio.ts and content-ai-service.ts converted (content-ideas, post-copy, post-copy-variations, campaign-content, image-prompt, shorten-copy, content-compress, content-suggest-levels, content-suggest-niche, brand-voice-generator, advisage-analyze, topic-suggest).
  - **Feature Map**: 40+ technical feature IDs mapped to Italian sidebar names across 8 categories (Principale, Lavoro Quotidiano, Comunicazione, Content Studio, Cervello AI, AI Avanzato, Cliente, Sistema) in consultant-ai-usage.tsx.
  - **by-client API fix**: Removed client_role from GROUP BY (was causing duplicate rows). Uses CASE WHEN for role determination. Added UNION with active_clients CTE to show all consultant clients even with 0 token usage.
  - **Dashboard improvements**: Dual Y-axis trend chart (cost + tokens), category aggregation bar chart, improved user table with "Tu" badge and translated topFeature, feature detail table with Italian names and category badges.
  - Key files: `server/ai/token-tracker.ts`, `server/ai/provider-factory.ts`, `server/routes/ai-usage-router.ts`, `client/src/pages/consultant-ai-usage.tsx`, `server/routes/content-studio.ts`, `server/services/content-ai-service.ts`.
- **[2026-02-16] File Search Routing to Private Client/Employee Stores:**
  - **New routing**: System prompt documents in File Search mode now sync directly to recipient private stores instead of consultant store.
  - **Routing map**: `consultant_only` → consultant store; `all` → all client+employee private stores; `clients_only` → non-employee client stores; `specific_clients` → specific client stores; `employees_only` → employee stores; `specific_departments` → department stores; `specific_employees` → specific employee stores. WhatsApp/Autonomous agents → dedicated stores (unchanged).
  - **New helpers**: `resolveClientAssistantStores()` resolves target stores by mode. `removeSystemPromptDocumentFromAllStores()` cleans all stores efficiently via fileSearchDocuments query.
  - **Read path**: `getStoreBreakdownForGeneration()` in `file-search-service.ts` now includes department stores for employees, so when an employee uses the AI chatbot, it searches both their private store and their department store.
  - **Confirmation dialog**: Frontend shows correct store destinations per routing mode. Removed outdated warning about consultant store for specific_clients.
  - Key files: `file-search-sync-service.ts`, `file-search-service.ts`, `knowledge-documents.ts`, `google-drive.ts`, `SystemDocumentsSection.tsx`.
- **[2026-02-16] Dedicated File Search Stores for Autonomous Agents & Departments:**
  - **Architecture**: Each autonomous AI agent (Marco, Alessia, Nova, etc.) now has a dedicated File Search store (ownerType='autonomous_agent', ownerId='agentId_consultantId') instead of sharing the consultant's store. This prevents cross-agent document visibility.
  - **Department stores**: Added ownerType='department' support for department-specific document stores.
  - Key files: `file-search-sync-service.ts`, `file-search-service.ts`, `knowledge-documents.ts`, `google-drive.ts`, `ai-task-executor.ts`.
- **[2026-02-15] Critical AI Employee Bug Fixes:**
  - **Marco contact redirect**: When `ai_role='marco'`, the scheduler now resolves consultant phone/whatsapp from `ai_autonomy_settings` and overrides `contact_phone`, `contact_name`, and clears `contact_id` so Marco messages the consultant (not clients). Changes persisted to DB for retry safety.
  - **WhatsApp agent routing**: Each AI role's `defaultWhatsappAgentId` from `agent_contexts` is resolved at execution time and passed as `agentConfigId` to `sendWhatsAppMessage`, preventing default-to-first-agent behavior.
  - **UI: WhatsApp agent selector**: Added per-employee WhatsApp agent dropdown in `AgentContextEditor` (SettingsTab), fetching active agents from `/api/whatsapp/agent-chat/agents`.
  - **Scheduling safeguards**: `computeTaskScheduledAt` now validates AI-suggested times against working days (not just hours) and falls back to next working slot if fallback date is in the past.
- Added quick save buttons for autonomy level and specific working hours in `SettingsTab.tsx` for each agent.
- Investigated WhatsApp "Assistenza Clienti" Twilio credentials error in `ai-task-executor.ts`.
