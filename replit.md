...
*   **Consultant Workflow:** A 4-phase, 27-step Consultant Setup Wizard guides configuration, alongside configurable monthly consultation limits. An integrated **Accademia di Formazione** LMS at `/consultant/academy` provides 27 lessons (one per wizard step) organized in 6 modules, with Guidde video embeds, progress tracking via DB (`consultant_academy_completions`), bidirectional wizard↔academy links, and automatic pre-completion for wizard-verified steps.
