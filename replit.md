...
- **[2026-02-16] Dedicated File Search Stores for Autonomous Agents & Departments:**
  ...
  - **Schema**: Updated `fileSearchStores.ownerType` union type to include 'autonomous_agent' and 'department'.
  - **Sync service**: Added `getOrCreateAutonomousAgentStore()`, `getOrCreateDepartmentStore()`, `getAutonomousAgentStore()`, `getDepartmentStore()` functions in `file-search-sync-service.ts`.
  - **CRUD routes**: All 4 operations (CREATE, UPDATE, TOGGLE, DELETE) in `knowledge-documents.ts` + Google Drive import now sync to dedicated agent stores per enabled agent, and to department stores for department-targeted docs. Full cleanup on delete/toggle-off including department stores.
  - **Read path**: `loadAgentDocuments()` in `ai-task-executor.ts` now loads agent's dedicated store (priority) + consultant stores for File Search queries.
  - **File Search service**: Added `autonomous_agent` and `department` cases to `resolveStoreCredentialOwner` in `file-search-service.ts`.
