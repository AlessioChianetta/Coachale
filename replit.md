...
- Content Autopilot System: Now integrated directly within the Ideas page (no standalone page). Features include: date range selection with past date blocking, platform-specific frequency (1-5 posts/day), 3 preset templates (Lancio Prodotto, Mantenimento, Campagna Aggressiva), SSE progress streaming, and automatic scheduling with calendar integration. Advanced filters: excludes weekends and Italian holidays (2024-2026 calendar), prevents duplicate generation for already-scheduled days with date range detection. Two generation modes: "Automatica" (batch all) and "Controllata" (day-by-day preview with regeneration). Content theme rotation (educativo, promozionale, storytelling, behind-the-scenes) with AI quality scoring (1-10) per post. Inherits configuration from Ideas page (platform, schema, writing style). Brand Assets Integration: loads posting schedule (times, writing style) from Brand Assets with fallback to default optimal times (Instagram: 11:00/14:00/19:00, X: 09:00/12:00/17:00, LinkedIn: 08:00/12:00/17:30). Shows "Configurazione Attiva" section displaying inherited settings with visual indicator showing source (Brand Assets vs default). Configured times are passed to the backend for content generation.
