...
- **Per-Agent Google Calendar Integration**: Each WhatsApp agent can have its own dedicated Google Calendar for appointments, preventing conflicts between agents. Agent calendars are configured in the AgentProfilePanel sidebar (not the wizard). Features include connection status display, email indicator, test connection button, and connect/disconnect functionality. Falls back to consultant's calendar if agent has no dedicated calendar. OAuth tokens stored per-agent in `consultant_whatsapp_config` table. Requires 5 OAuth redirect URIs in Google Cloud Console (Drive x2, Calendar x2, plus JS origin).
