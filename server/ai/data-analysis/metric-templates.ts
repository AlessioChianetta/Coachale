...
export const METRIC_TEMPLATES: Record<string, MetricTemplate> = {
  ...
  gross_margin: {
    ...
    sqlTemplate: 'SUM(CAST({total_net} AS NUMERIC)) - SUM(CAST({cost} AS NUMERIC) * CAST({quantity} AS NUMERIC))',
    requiredLogicalColumns: ["total_net", "cost", "quantity"],
    unit: "currency",
    validationRules: {},
    isPrimary: true,
    version: 1,
  ...
  gross_margin_percent: {
    ...
    sqlTemplate: '((SUM(CAST({total_net} AS NUMERIC)) - SUM(CAST({cost} AS NUMERIC) * CAST({quantity} AS NUMERIC))) / NULLIF(SUM(CAST({total_net} AS NUMERIC)), 0)) * 100',
    requiredLogicalColumns: ["total_net", "cost", "quantity"],
    unit: "percentage",
    validationRules: {
      minValue: 0,
  ...
  discount_total: {
    ...
    sqlTemplate: 'SUM(CAST({total_net} AS NUMERIC) * (CAST({discount_percent} AS NUMERIC) / 100))',
    requiredLogicalColumns: ["total_net", "discount_percent"],
    unit: "currency",
    validationRules: {
      mustBePositive: true,
      minValue: 0,
