...
export const METRIC_TEMPLATES: Record<string, MetricTemplate> = {
  ...
  // ============================================
  cost_staff_total: {
    name: "cost_staff_total",
    displayName: "Costo Personale Totale",
    description: "Somma del costo del personale di produzione",
    sqlTemplate: 'SUM(CAST({cost_staff} AS NUMERIC))',
    requiredLogicalColumns: ["cost_staff"],
    unit: "currency",
  ...
  cost_staff_percent: {
    name: "cost_staff_percent",
    displayName: "Incidenza Personale %",
    description: "Percentuale del costo personale sul fatturato",
    sqlTemplate: '(SUM(CAST({cost_staff} AS NUMERIC)) / NULLIF(SUM(CAST({revenue_amount} AS NUMERIC)), 0)) * 100',
    requiredLogicalColumns: ["cost_staff", "revenue_amount"],
    unit: "percentage",
    validationRules: { minValue: 0, maxValue: 100, warningThreshold: 40, warningMessage: "Costo personale superiore al 40% del fatturato" },
    isPrimary: true,
    version: 1,
  },
  cost_foodbev_total: {
    name: "cost_foodbev_total",
    displayName: "Costo F&B Totale",
    description: "Somma del costo food & beverage di produzione",
    sqlTemplate: 'SUM(CAST({cost_foodbev} AS NUMERIC))',
    requiredLogicalColumns: ["cost_foodbev"],
    unit: "currency",
    validationRules: { minValue: 0 },
    isPrimary: true,
    version: 1,
  ...
  cost_foodbev_percent: {
    name: "cost_foodbev_percent",
    displayName: "Incidenza F&B %",
    description: "Percentuale del costo food & beverage sul fatturato",
    sqlTemplate: '(SUM(CAST({cost_foodbev} AS NUMERIC)) / NULLIF(SUM(CAST({revenue_amount} AS NUMERIC)), 0)) * 100',
    requiredLogicalColumns: ["cost_foodbev", "revenue_amount"],
    unit: "percentage",
  ...
  cost_materials_total: {
    name: "cost_materials_total",
    displayName: "Costo Materiali Totale",
    description: "Somma del costo materiali di produzione",
    sqlTemplate: 'SUM(CAST({cost_materials} AS NUMERIC))',
    requiredLogicalColumns: ["cost_materials"],
    unit: "currency",
    validationRules: { minValue: 0 },
    isPrimary: false,
    version: 1,
  ...
  cost_extra_total: {
    ...
    displayName: "Costo Extra Totale",
    description: "Somma dei costi extra di produzione",
    sqlTemplate: 'SUM(CAST({cost_extra} AS NUMERIC))',
    requiredLogicalColumns: ["cost_extra"],
    unit: "currency",
    validationRules: { minValue: 0 },
    isPrimary: false,
    version: 1,
  ...
  cost_structure_total: {
    ...
    displayName: "Costo Struttura Totale",
    description: "Somma del costo struttura di produzione",
    sqlTemplate: 'SUM(CAST({cost_structure} AS NUMERIC))',
    requiredLogicalColumns: ["cost_structure"],
    unit: "currency",
    validationRules: { minValue: 0 },
    isPrimary: false,
    version: 1,
  ...
  cost_utilities_total: {
    name: "cost_utilities_total",
    displayName: "Costo Consumi Totale",
    description: "Somma del costo consumi/utenze di produzione",
    sqlTemplate: 'SUM(CAST({cost_utilities} AS NUMERIC))',
    requiredLogicalColumns: ["cost_utilities"],
    unit: "currency",
    validationRules: { minValue: 0 },
    isPrimary: false,
    version: 1,
  ...
  prime_cost: {
    ...
    description: "Food cost (materie prime) + costo personale. Standard di settore ristorazione.",
    sqlTemplate: 'SUM(CAST({cost} AS NUMERIC) * CAST({quantity} AS NUMERIC)) + SUM(CAST({cost_staff} AS NUMERIC))',
    requiredLogicalColumns: ["cost", "quantity", "cost_staff"],
    unit: "currency",
    validationRules: { minValue: 0 },
    isPrimary: true,
    version: 1,
  ...
  prime_cost_percent: {
    name: "prime_cost_percent",
    displayName: "Prime Cost %",
    description: "Percentuale prime cost sul fatturato. Benchmark: <65% ottimale, 65-70% accettabile, >70% critico.",
    sqlTemplate: '((SUM(CAST({cost} AS NUMERIC) * CAST({quantity} AS NUMERIC)) + SUM(CAST({cost_staff} AS NUMERIC))) / NULLIF(SUM(CAST({revenue_amount} AS NUMERIC)), 0)) * 100',
    requiredLogicalColumns: ["cost", "quantity", "cost_staff", "revenue_amount"],
    unit: "percentage",
    validationRules: { minValue: 0, maxValue: 100, warningThreshold: 70, warningMessage: "Prime cost superiore al 70% - margini molto bassi" },
    isPrimary: true,
    version: 1,
  },
  total_production_cost: {
    name: "total_production_cost",
    displayName: "Costo Produzione Totale",
    description: "Somma di tutti i costi di produzione (food cost + F&B + materiali + extra + personale + struttura + consumi)",
    sqlTemplate: 'SUM(CAST({cost} AS NUMERIC) * CAST({quantity} AS NUMERIC)) + SUM(CAST({cost_foodbev} AS NUMERIC)) + SUM(CAST({cost_materials} AS NUMERIC)) + SUM(CAST({cost_extra} AS NUMERIC)) + SUM(CAST({cost_staff} AS NUMERIC)) + SUM(CAST({cost_structure} AS NUMERIC)) + SUM(CAST({cost_utilities} AS NUMERIC))',
    requiredLogicalColumns: ["cost", "quantity", "cost_foodbev", "cost_materials", "cost_extra", "cost_staff", "cost_structure", "cost_utilities"],
    unit: "currency",
    validationRules: { minValue: 0 },
    isPrimary: true,
    version: 1,
  ...
  total_production_cost_percent: {
    name: "total_production_cost_percent",
    displayName: "Incidenza Costi Produzione %",
    description: "Percentuale costi produzione totali sul fatturato",
    sqlTemplate: '((SUM(CAST({cost} AS NUMERIC) * CAST({quantity} AS NUMERIC)) + SUM(CAST({cost_foodbev} AS NUMERIC)) + SUM(CAST({cost_materials} AS NUMERIC)) + SUM(CAST({cost_extra} AS NUMERIC)) + SUM(CAST({cost_staff} AS NUMERIC)) + SUM(CAST({cost_structure} AS NUMERIC)) + SUM(CAST({cost_utilities} AS NUMERIC))) / NULLIF(SUM(CAST({revenue_amount} AS NUMERIC)), 0)) * 100',
    requiredLogicalColumns: ["cost", "quantity", "cost_foodbev", "cost_materials", "cost_extra", "cost_staff", "cost_structure", "cost_utilities", "revenue_amount"],
    unit: "percentage",
    validationRules: { minValue: 0, maxValue: 100 },
    isPrimary: false,
    version: 1,
