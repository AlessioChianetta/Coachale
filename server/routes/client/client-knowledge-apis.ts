...
import {
  ...
  insertClientKnowledgeApiSchema,
  updateClientKnowledgeApiSchema,
  users,
} from "../../../shared/schema";
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const apis = await db
        ...
        .orderBy(desc(clientKnowledgeApis.createdAt));
      ...
      console.error("❌ [CLIENT KNOWLEDGE APIS] Error listing APIs:", error);
      res.status(500).json({
        success: false,
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const cacheEntries = await db
        ...
        .where(eq(clientKnowledgeApiCache.apiConfigId, id))
        .orderBy(desc(clientKnowledgeApiCache.updatedAt));
      ...
      console.error("❌ [CLIENT KNOWLEDGE APIS] Error fetching API:", error);
      res.status(500).json({
        success: false,
...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const validatedData = insertClientKnowledgeApiSchema.parse({
        ...req.body,
        clientId,
      });
      ...
      console.error("❌ [CLIENT KNOWLEDGE APIS] Error creating API:", error);
...
router.put(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const validatedData = updateClientKnowledgeApiSchema.parse(req.body);
      ...
      const [updatedApi] = await db
        ...
        .set(updateData)
        .where(eq(clientKnowledgeApis.id, id))
        .returning();
      ...
      console.error("❌ [CLIENT KNOWLEDGE APIS] Error updating API:", error);
...
router.delete(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      await db
        .delete(clientKnowledgeApiCache)
        .where(eq(clientKnowledgeApiCache.apiConfigId, id));

      await db
        .delete(clientKnowledgeApis)
        .where(eq(clientKnowledgeApis.id, id));
      ...
      console.error("❌ [CLIENT KNOWLEDGE APIS] Error deleting API:", error);
      res.status(500).json({
        success: false,
...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      console.error("❌ [CLIENT KNOWLEDGE APIS] Test connection error:", error);
      ...
      await db
        ...
        .where(eq(clientKnowledgeApis.id, req.params.id));
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const [existingCache] = await db
        ...
        .where(
          and(
            eq(clientKnowledgeApiCache.apiConfigId, id),
            eq(clientKnowledgeApiCache.cacheKey, cacheKey)
          )
        )
        .limit(1);
      ...
      if (existingCache) {
        await db
          ...
          .where(eq(clientKnowledgeApiCache.id, existingCache.id));
      } else {
        await db.insert(clientKnowledgeApiCache).values({
          apiConfigId: id,
          clientId,
          cacheKey,
          cachedData: response.data,
      ...
      console.error("❌ [CLIENT KNOWLEDGE APIS] Sync error:", error);
