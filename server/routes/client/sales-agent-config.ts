...
import { insertClientSalesAgentSchema, updateClientSalesAgentSchema, salesConversationTraining, salesAgentTrainingSummary } from "@shared/schema";
import { z } from "zod";
...
router.get("/script-structure", authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    console.log(`[ScriptStructure] GET script structure`);

    // Load script structure JSON
    ...
    if (!fs.existsSync(scriptStructurePath)) {
      console.error(`[ScriptStructure] File not found at ${scriptStructurePath}`);
      return res.status(404).json({ 
        message: "Script structure file not found. Run parser to generate it.",
    ...
    console.log(`[ScriptStructure] Loaded script structure v${scriptStructure.version} with ${scriptStructure.metadata.totalPhases} phases`);
