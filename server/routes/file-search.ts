  ...
  try {
    ...
    const consultantStore = {
      ...
      documents: {
        ...
        opClients: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_clients_')),
        opClientStates: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_clientstates_')),
        opWhatsappTemplates: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_whatsapptemplates_')),
        opTwilioTemplates: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_twiliotemplates_')),
        opConfig: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_config_')),
        opEmail: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_email_')),
        opCampaigns: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_campaigns_')),
        opCalendar: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_calendar_')),
        opExercisesPending: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_exercisespending_')),
        opConsultations: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_consultations_')),
      ...
      totals: {
        ...
        opClients: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_clients_')).length,
        opClientStates: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_clientstates_')).length,
        opWhatsappTemplates: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_whatsapptemplates_')).length,
        opTwilioTemplates: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_twiliotemplates_')).length,
        opConfig: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_config_')).length,
        opEmail: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_email_')).length,
        opCampaigns: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_campaigns_')).length,
        opCalendar: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_calendar_')).length,
        opExercisesPending: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_exercisespending_')).length,
        opConsultations: consultantDocs.filter(d => d.sourceType === 'operational_context' && d.sourceId?.startsWith('operational_consultations_')).length,
      },
    };
  ...
  try {
    ...
    switch (type) {
      case 'library':
        ...
        result = await fileSearchSyncService.syncLibraryDocument(id, consultantId);
        break;
      case 'knowledge_base':
        // FIX: Parameters were inverted - documentId comes first, then consultantId
      ...
      case 'exercise':
        ...
        result = await fileSearchSyncService.syncExercise(id, consultantId);
        break;
      case 'university_lesson':
        // FIX: Parameters were inverted - lessonId comes first, then consultantId
        result = await fileSearchSyncService.syncUniversityLesson(id, consultantId);
        break;
      case 'assigned_exercise':
        // Sync an exercise to a specific client's private store
        if (!clientId) {
          return res.status(400).json({ error: 'clientId is required for assigned_exercise' });
        }
      ...
      case 'assigned_library_category':
        ...
        result = { 
          success: libSuccess, 
          error: libResult.failed > 0 ? `${libResult.failed} documenti non sincronizzati` : libResult.error 
        };
        break;
      ...
      case 'assigned_university_year':
        ...
        result = { 
          success: uniSuccess, 
          error: uniResult.failed > 0 ? `${uniResult.failed} lezioni non sincronizzate` : uniResult.error 
        };
        break;
